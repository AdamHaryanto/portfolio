...
import { Menu, X, Download, ExternalLink, Mail, Phone, Instagram, Linkedin, Github, Pencil, RotateCcw, Check, Plus, Trash2, Ban, Send, Link as LinkIcon, ChevronDown, Settings, Video, AlertTriangle } from 'lucide-react';
...
function App() {
  ...
  // Session Management
  const [appKey, setAppKey] = useState(0); // Used to force full re-render on cancel
  ...
  // Helper to ensure IDs exist (for migration)
  ...
  useEffect(() => {
    ...
    if (savedArt) {
      ...
      if (old3D || old2D) {
        ...
        if (old3D) {
          try {
             const urls = JSON.parse(old3D);
             newCats[0].items = urls.map((u: string, i: number) => ({ id: `3d_mig_${i}`, url: u, type: 'image' }));
          } catch(e) {}
        }
        if (old2D) {
           try {
             const urls = JSON.parse(old2D);
             newCats[1].items = urls.map((u: string, i: number) => ({ id: `2d_mig_${i}`, url: u, type: 'image' }));
           } catch(e) {}
        }
  ...
  const startEditMode = () => {
    ...
    // Backup all editable component keys from localStorage
    Object.keys(localStorage).forEach(key => {
      if (key.startsWith('text_') || key.startsWith('img_') || key.startsWith('media_')) {
  ...
  const cancelEditMode = () => {
    // If no backup exists (shouldn't happen if entered via code), just exit
    if (!backupRef.current) {
      setIsEditMode(false);
      return;
    }

    // Direct revert without confirmation dialog as per user request for immediate action
    const snap = backupRef.current;
    
    // 1. Restore State variables
    setDynamicSkills(snap.skills);
    setDynamicProjects(snap.projects);
    setDynamicExperiences(snap.experiences);
    setDynamicCertificates(snap.certificates);
    setArtCategories(snap.artCategories);

    // 2. Restore State to LocalStorage (Arrays)
    save('user_skills', snap.skills);
    save('user_projects', snap.projects);
    save('user_experiences', snap.experiences);
    save('user_certificates', snap.certificates);
    save('user_art_categories', snap.artCategories);

    // 3. Restore Individual Editable Components (Text/Images)
    // First, delete keys created during this session
    Object.keys(localStorage).forEach(key => {
      if ((key.startsWith('text_') || key.startsWith('img_') || key.startsWith('media_')) && !snap.localStorage.hasOwnProperty(key)) {
    ...
    // 4. Force Update
    // Dispatch event for components listening to revert
    window.dispatchEvent(new Event('revert-data'));
    // Force Remount of App content to reset all child components cleanly
  ...
  const updateProjectMedia = (pIndex: number, mediaType: 'main' | 'screenshot', url: string, sIndex?: number) => {
     const updated = [...dynamicProjects];
     if (mediaType === 'main') {
        updated[pIndex].image = url;
     } else if (typeof sIndex === 'number') {
        updated[pIndex].screenshots[sIndex] = url;
     }
     setDynamicProjects(updated);
     save('user_projects', updated);
  };
  ...
  const addExperience = () => {
     ...
     const updated = [...dynamicExperiences, newExp];
  ...
  const addCertificate = () => {
    ...
    const updated = [...dynamicCertificates, newCert];
  ...
  // --- ART PORTFOLIO FUNCTIONS ---
  const addArtCategory = () => {
     const newCat: ArtCategory = {
        id: `cat_${Date.now()}`,
        title: "New Portfolio Group",
        items: []
     };
     const updated = [...artCategories, newCat];
  ...
  return (
    <>
      ...
      <div key={appKey} className={`min-h-screen font-sans selection:bg-brand-orange selection:text-white pb-20 relative ${showIntro ? 'overflow-hidden h-screen' : ''}`}>
        <BackgroundAnimation />
        <nav className="sticky top-0 z-50 bg-brand-bg/95 backdrop-blur-sm border-b-4 border-brand-dark py-3 px-4 md:px-8">
          <div className="max-w-7xl mx-auto flex justify-between items-center">
            <div className="flex items-center gap-4">
              <a href="#" onClick={(e) => scrollToSection(e, '#')} className="font-black text-xl tracking-tighter border-2 border-brand-dark px-3 py-1 rounded-lg bg-white shadow-retro-sm">
                Adam Haryanto - Portfolio
              </a>
            </div>
            
            <div className="hidden md:flex items-center space-x-6">
              <a href="#about" onClick={(e) => scrollToSection(e, '#about')} className="font-bold text-brand-dark hover:text-brand-orange transition-colors hover:underline decoration-4 underline-offset-4">About</a>
              <a href="#education" onClick={(e) => scrollToSection(e, '#education')} className="font-bold text-brand-dark hover:text-brand-orange transition-colors hover:underline decoration-4 underline-offset-4">Education</a>
              <a href="#experience" onClick={(e) => scrollToSection(e, '#experience')} className="font-bold text-brand-dark hover:text-brand-orange transition-colors hover:underline decoration-4 underline-offset-4">Experience</a>
              <a href="#skills" onClick={(e) => scrollToSection(e, '#skills')} className="font-bold text-brand-dark hover:text-brand-orange transition-colors hover:underline decoration-4 underline-offset-4">Skills</a>
              
              <div className="relative group">
                <button 
                  className="flex items-center gap-1 font-bold text-brand-dark hover:text-brand-orange transition-colors hover:underline decoration-4 underline-offset-4"
                  onClick={() => setPortfolioDropdownOpen(!portfolioDropdownOpen)}
                >
                  Portfolio <ChevronDown size={16}/>
                </button>
                {(portfolioDropdownOpen) && (
                  <div className="absolute top-full left-0 mt-2 w-48 bg-white border-2 border-brand-dark shadow-retro rounded-lg overflow-hidden flex flex-col">
                     <a href="#portfolio" onClick={(e) => scrollToSection(e, '#portfolio')} className="px-4 py-2 font-bold hover:bg-brand-orange hover:text-white border-b-2 border-brand-dark/10">Project Portfolio</a>
                     <a href="#art-portfolio" onClick={(e) => scrollToSection(e, '#art-portfolio')} className="px-4 py-2 font-bold hover:bg-brand-green hover:text-white">Art Portfolio</a>
                  </div>
                )}
              </div>

              <a href="#contact" onClick={(e) => scrollToSection(e, '#contact')} className="font-bold text-brand-dark hover:text-brand-orange transition-colors hover:underline decoration-4 underline-offset-4">Contact</a>
              
              {isEditMode && (
                <div className="flex items-center gap-2 ml-4 pl-4 border-l-2 border-brand-dark/20">
                  ...
                  <button onClick={() => setIsEditMode(false)} className="flex items-center gap-2 px-3 py-1.5 rounded-full font-bold border-2 transition-all bg-brand-green text-white border-brand-dark shadow-retro-sm">
            ...
            <div className="flex md:hidden gap-2">
              {isEditMode && (
                 <button onClick={() => setIsEditMode(false)} className="p-2 rounded-md border-2 bg-brand-green border-brand-dark text-white">
                   <Check size={20}/>
                 </button>
              )}
              <button className="p-2 border-2 border-brand-dark rounded-md bg-white active:bg-gray-100" onClick={() => setIsMenuOpen(!isMenuOpen)}>
          ...
          {isMenuOpen && (
            <div className="md:hidden absolute top-full left-0 w-full bg-brand-bg border-b-4 border-brand-dark shadow-xl">
              <div className="flex flex-col p-4 space-y-4">
                  <a href="#about" onClick={(e) => scrollToSection(e, '#about')} className="font-bold text-lg text-brand-dark block border-b-2 border-dashed border-brand-dark/20 pb-2">About</a>
                  <a href="#education" onClick={(e) => scrollToSection(e, '#education')} className="font-bold text-lg text-brand-dark block border-b-2 border-dashed border-brand-dark/20 pb-2">Education</a>
                  <a href="#experience" onClick={(e) => scrollToSection(e, '#experience')} className="font-bold text-lg text-brand-dark block border-b-2 border-dashed border-brand-dark/20 pb-2">Experience</a>
                  <a href="#skills" onClick={(e) => scrollToSection(e, '#skills')} className="font-bold text-lg text-brand-dark block border-b-2 border-dashed border-brand-dark/20 pb-2">Skills</a>
                  <div className="block border-b-2 border-dashed border-brand-dark/20 pb-2">
                     <span className="font-bold text-lg text-brand-dark block mb-2">Portfolio</span>
                     <div className="pl-4 flex flex-col gap-2">
                        <a href="#portfolio" onClick={(e) => scrollToSection(e, '#portfolio')} className="text-brand-dark/80 font-bold">Project Portfolio</a>
                        <a href="#art-portfolio" onClick={(e) => scrollToSection(e, '#art-portfolio')} className="text-brand-dark/80 font-bold">Art Portfolio</a>
                     </div>
                  </div>
                  <a href="#contact" onClick={(e) => scrollToSection(e, '#contact')} className="font-bold text-lg text-brand-dark block border-b-2 border-dashed border-brand-dark/20 pb-2">Contact</a>
        ...
        <div className="min-h-[80vh] flex flex-col justify-center items-center px-4 py-12">
          ...
          <Card className="p-8 md:p-12 max-w-md w-full text-center" variant="green">
            ...
            <EditableText initialText="Game Developer & Technical Artist" storageKey="hero_subtitle" isEditing={isEditMode} tag="p" className="font-bold text-xl mb-8 opacity-80" />
            <div className="flex flex-col gap-4">
              ...
              <a href={SOCIAL_LINKS.itch} target="_blank" rel="noreferrer" className="w-full">
                <Button fullWidth variant="outline" className="bg-white">Visit Itch.io</Button>
              </a>
            </div>
          </Card>
        </div>
            ...
            <div className="md:col-span-2 space-y-6">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <Card variant="orange" className="p-6 flex flex-col justify-center">
                  <span className="text-sm font-bold opacity-70 mb-1">Role</span>
                ...
                <Card variant="white" className="p-6 flex flex-col justify-center">
                  <EditableText initialText='"Finish what you start"' storageKey="motto_card" isEditing={isEditMode} tag="h3" className="text-2xl font-bold italic text-brand-dark" />
                  <span className="text-sm font-bold opacity-50 mt-2 text-right">- My Motto</span>
                </Card>
              </div>
          ...
          <div className="space-y-8">
            {EDUCATION.map((edu, index) => (
              <Card key={index} variant={index === 0 ? 'blue' : 'orange'} className="p-6 md:p-8 flex flex-col md:flex-row gap-6 items-start md:items-center">
                ...
                <div className="bg-brand-dark/10 p-6 rounded-xl border-2 border-brand-dark min-w-[150px] text-center">
        ...
        <Section id="experience" title="Experience" isEditing={isEditMode} storageKey="title_experience">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
              ...
              <div key={exp.id} className="relative group/exp">
                <Card variant="white" className="flex flex-col h-full" noShadow={false}>
                  ...
                  <div className="p-6 md:p-8 flex flex-col flex-grow">
                    ...
                    <div className="mb-6 pb-6 border-b-2 border-dashed border-gray-300/50">
                      {isEditMode ? (
                          <div className="flex flex-col gap-2">
                             <select value={exp.type} onChange={(e) => updateExperienceType(index, e.target.value as any)} className="text-xs border-2 border-brand-dark rounded p-1 w-max">
                                <option value="Work">Work</option>
                                <option value="Organization">Organization</option>
                             </select>
                             <div className="flex gap-1">
                                <span className="text-xs font-bold">Role:</span>
                                <EditableText initialText={exp.role} storageKey={`exp_role_${exp.id}`} isEditing={true} tag="span" className="text-xs border-b border-brand-dark" />
                             </div>
                          </div>
                      ) : (
                          <div className={`inline-block px-3 py-1 rounded-full text-xs font-bold border-2 border-brand-dark ${exp.type === 'Work' ? 'bg-brand-green' : 'bg-brand-orange'}`}>{exp.role}</div>
                      )}
                    </div>
                    ...
                    <div className="bg-brand-yellow/30 p-4 rounded-lg border-2 border-brand-dark">
            ...
            {isEditMode && (
               <button onClick={addExperience} className="w-full min-h-[300px] border-4 border-dashed border-brand-dark/30 rounded-xl p-8 flex flex-col items-center justify-center text-brand-dark/50 hover:bg-brand-dark/5 hover:border-brand-dark hover:text-brand-dark transition-all group">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
            {dynamicSkills.map((category, catIndex) => (
              <div key={catIndex} className="flex flex-col gap-4">
                <div className={`p-4 border-4 border-brand-dark rounded-xl shadow-retro text-center font-black text-xl uppercase ${catIndex === 0 ? 'bg-brand-orange' : catIndex === 1 ? 'bg-brand-blue' : 'bg-brand-yellow'}`}>
                ...
                <div className="flex flex-col gap-3 relative pb-4">
                   <div className="absolute left-1/2 top-0 bottom-12 w-1 bg-brand-dark/20 -translate-x-1/2 -z-10 border-l-2 border-dashed border-brand-dark"></div>
                       ...
                       {isEditMode ? (
                          <div className="flex gap-2 items-center">
                            <Card variant="white" className="flex-grow py-3 px-2 text-center font-bold text-sm" noShadow>
                               <input value={skill} onChange={(e) => updateSkill(catIndex, sIndex, e.target.value)} className="w-full text-center bg-transparent focus:outline-none" />
                            </Card>
        ...
        <Section id="portfolio" title="Project Portfolio" isEditing={isEditMode} storageKey="title_projects">
          <div className="space-y-12">
                ...
                <Card variant="white" className="p-6 md:p-8">
                  <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
                    <div className="lg:col-span-7 flex flex-col gap-4">
                      <div className="border-4 border-brand-dark rounded-xl overflow-hidden shadow-sm aspect-video bg-black/5">
                        <EditableMedia src={project.image} alt={project.title} className="w-full h-full object-cover hover:scale-105 transition-transform duration-500" storageKey={`project_${project.id}_main`} isEditing={isEditMode} onUpdate={(newUrl) => updateProjectMedia(index, 'main', newUrl)} />
                      </div>
                      <div className="flex gap-4 overflow-x-auto pb-4 no-scrollbar snap-x snap-mandatory -mx-2 px-2">
                        {project.screenshots.map((shot, sIdx) => (
                           <div key={sIdx} className="min-w-[45%] md:min-w-[45%] flex-shrink-0 relative group/shot snap-start">
                              <div className="border-4 border-brand-dark rounded-xl overflow-hidden aspect-video h-full bg-black/5">
                                <EditableMedia src={shot} alt="Screenshot" className="w-full h-full object-cover" storageKey={`project_${project.id}_shot_${sIdx}`} isEditing={isEditMode} onUpdate={(newUrl) => updateProjectMedia(index, 'screenshot', newUrl, sIdx)} />
                              </div>
                              {isEditMode && (
                        ...
                        {isEditMode && (
                          <button onClick={() => addScreenshot(index)} className="min-w-[100px] flex-shrink-0 border-4 border-dashed border-brand-dark/30 rounded-xl flex items-center justify-center text-brand-dark/50 hover:bg-brand-dark/5 hover:border-brand-dark hover:text-brand-dark transition-all snap-start">
                            <Plus size={32} />
                          </button>
                        )}
                      </div>
                    ...
                    <div className="lg:col-span-5 flex flex-col gap-6">
                      <div className="bg-brand-blue p-6 rounded-xl border-4 border-brand-dark shadow-retro-sm">
                        <div className="flex justify-between items-start mb-2">
                           <div className="font-bold text-sm opacity-70 flex flex-wrap gap-1 items-center">
                        ...
                        <EditableText initialText={project.title} storageKey={`proj_title_${project.id}`} isEditing={isEditMode} tag="h3" className="text-3xl font-black mb-4" />
                        <EditableText initialText={project.description} storageKey={`proj_desc_${project.id}`} isEditing={isEditMode} tag="p" multiline={true} className="font-medium text-sm leading-relaxed mb-6" />
                        <div className="flex justify-end gap-2 items-center">
                           {isEditMode ? (
                             <select value={project.status} onChange={(e) => updateProjectField(index, 'status', e.target.value)} className="bg-white border-2 border-brand-dark rounded px-2 py-1 text-xs font-bold">
                               <option value="Prototype">Prototype</option>
                               <option value="WIP">WIP</option>
                               <option value="Released">Released</option>
                             </select>
                           ) : (
                             <span className="bg-white border-2 border-brand-dark px-3 py-1 rounded-full text-xs font-bold uppercase">Status: {project.status}</span>
                           )}
                        </div>
                      </div>
                      ...
                      {isEditMode && (
                        <div className="bg-white p-2 border-2 border-brand-dark rounded mb-2">
            ...
            {isEditMode && (
               <button onClick={addProject} className="w-full border-4 border-dashed border-brand-dark/30 rounded-xl p-8 flex flex-col items-center justify-center text-brand-dark/50 hover:bg-brand-dark/5 hover:border-brand-dark hover:text-brand-dark transition-all group">
                  <Plus size={48} className="mb-2 group-hover:scale-110 transition-transform" />
        ...
        <Section id="art-portfolio" title="Art Portfolio" isEditing={isEditMode} storageKey="title_art">
          <div className="space-y-16">
            {artCategories.map((category, catIndex) => (
              <div key={category.id} className="relative group/category">
                 <div className="flex justify-between items-end relative">
                    <div className={`text-brand-dark inline-block px-6 py-2 rounded-t-xl border-x-4 border-t-4 border-brand-dark font-black text-xl ${catIndex % 3 === 0 ? 'bg-brand-orange text-white' : catIndex % 3 === 1 ? 'bg-brand-green' : 'bg-brand-blue'}`}>
                    ...
                    {isEditMode && (
                       <div className="flex gap-2 relative z-50 transform translate-y-2">
                          <button onClick={() => addArtItem(catIndex)} className="bg-brand-yellow text-brand-dark px-3 py-1 rounded-t-lg font-bold border-t-2 border-x-2 border-brand-dark flex gap-2 items-center text-sm hover:scale-105 transition-transform"><Plus size={14}/> Add Media</button>
                 ...
                 <Card variant="white" className="p-6 rounded-tl-none relative z-40" disableHover>
                   <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                     {category.items.map((item, itemIndex) => (
                       <div key={item.id} className="group relative border-4 border-brand-dark rounded-xl overflow-hidden aspect-video bg-black/5">
                         <EditableMedia src={item.url} alt={`${category.title} ${itemIndex + 1}`} className="w-full h-full object-cover transition-transform duration-500 group-hover:scale-105" storageKey={`art_item_${item.id}`} isEditing={isEditMode} onUpdate={(newUrl) => updateArtItemUrl(catIndex, itemIndex, newUrl)} />
                     ...
                     {category.items.length === 0 && (
                        <div className="col-span-full py-12 text-center opacity-50 font-bold border-2 border-dashed border-brand-dark rounded-xl">No items yet. Click "Add Media" to start!</div>
                     )}
                   </div>
                 </Card>
              </div>
            ...
            {isEditMode && (
               <button onClick={addArtCategory} className="w-full border-4 border-dashed border-brand-dark/30 rounded-xl p-8 flex flex-col items-center justify-center text-brand-dark/50 hover:bg-brand-dark/5 hover:border-brand-dark hover:text-brand-dark transition-all group">
                  <Plus size={48} className="mb-2 group-hover:scale-110 transition-transform" />
        ...
        <Section id="certificates" title="Certificates" isEditing={isEditMode} storageKey="title_certs">
           <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
              {dynamicCertificates.map((cert, index) => (
                  <div key={cert.id} className="relative group/cert">
                    <Card variant="white" className="p-4 group cursor-pointer hover:-translate-y-1 transition-transform" noShadow={false}>
                        <div className="border-2 border-brand-dark rounded-lg overflow-hidden aspect-[4/3] mb-4 relative" onClick={() => setSelectedCertificate(cert)}>
                        <EditableImage src={cert.image} alt={cert.title} className="w-full h-full object-cover grayscale group-hover:grayscale-0 transition-all" storageKey={`cert_img_${cert.id}`} isEditing={isEditMode} />
                        </div>
                        <div className={`p-3 rounded-lg border-2 border-brand-dark text-center font-bold ${index % 2 === 0 ? 'bg-brand-orange' : 'bg-brand-blue'}`}>
                          <EditableText initialText={cert.title} storageKey={`cert_title_${cert.id}`} isEditing={isEditMode} tag="span" />
                        </div>
                        {isEditMode && (
                            <div className="mt-2 text-xs text-center border-t border-dashed border-gray-400 pt-2">
              ...
              {isEditMode && (
                  <button onClick={addCertificate} className="min-h-[250px] border-4 border-dashed border-brand-dark/30 rounded-xl flex flex-col items-center justify-center text-brand-dark/50 hover:bg-brand-dark/5 hover:border-brand-dark hover:text-brand-dark transition-all">
                     <Plus size={32} />
        ...
        {selectedCertificate && (
            <div className="fixed inset-0 z-[100] bg-black/80 backdrop-blur-sm flex items-center justify-center p-4" onClick={() => setSelectedCertificate(null)}>
                <div className="bg-brand-bg p-2 rounded-xl border-4 border-brand-dark max-w-3xl w-full shadow-retro relative animate-bounce-in" onClick={e => e.stopPropagation()}>
                    <button onClick={() => setSelectedCertificate(null)} className="absolute -top-4 -right-4 bg-brand-red text-white p-2 rounded-full border-2 border-brand-dark shadow-retro-sm hover:scale-110 transition-transform z-10">
                        <X size={24}/>
                    </button>
                    <div className="border-2 border-brand-dark rounded-lg overflow-hidden">
                    ...
                    <div className="p-4 text-center">
                        <h3 className="text-2xl font-black text-brand-dark mb-1">{selectedCertificate.title}</h3>
                        <p className="font-bold text-lg text-brand-dark/70">{selectedCertificate.issuer} • {selectedCertificate.date}</p>
                    </div>
                </div>
            </div>
        )}
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
            <div className="lg:col-span-2 space-y-6">
               <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <a href={SOCIAL_LINKS.instagram} target="_blank" rel="noreferrer">
                    <Card variant="blue" className="p-4 flex items-center justify-between hover:bg-[#5AC5E3] cursor-pointer">
                  ...
                  <a href={SOCIAL_LINKS.whatsapp} target="_blank" rel="noreferrer">
                    <Card variant="orange" className="p-4 flex items-center justify-between hover:bg-[#FF8E52] cursor-pointer">
                  ...
                  <a href={`mailto:${SOCIAL_LINKS.email}`}>
                    <Card variant="yellow" className="p-4 flex items-center justify-between hover:bg-[#FCE06D] cursor-pointer">
                  ...
                  <a href={SOCIAL_LINKS.discord} target="_blank" rel="noreferrer">
                    <Card variant="green" className="p-4 flex items-center justify-between hover:bg-[#68D2AD] cursor-pointer">
                        <div className="flex items-center gap-4">
               ...
               <Card variant="white" className="p-6">
                  <h3 className="text-xl font-black mb-4 uppercase flex items-center gap-2"><Mail size={24}/> Send me an email</h3>
                  <form onSubmit={handleContactSubmit} className="space-y-4">
                    <div>
                      <label className="block text-xs font-bold uppercase mb-1">Your Name</label>
                      <input type="text" required value={contactForm.name} onChange={(e) => handleContactFormChange('name', e.target.value)} className="w-full border-2 border-brand-dark p-2 rounded-lg bg-brand-bg focus:outline-none focus:ring-2 focus:ring-brand-orange" placeholder="John Doe" />
                    </div>
                    <div>
                      <label className="block text-xs font-bold uppercase mb-1">Your Email</label>
                      <input type="email" required value={contactForm.email} onChange={(e) => handleContactFormChange('email', e.target.value)} className="w-full border-2 border-brand-dark p-2 rounded-lg bg-brand-bg focus:outline-none focus:ring-2 focus:ring-brand-orange" placeholder="john@example.com" />
                    </div>
                    <div>
                      <label className="block text-xs font-bold uppercase mb-1">Message</label>
                      <textarea required value={contactForm.message} onChange={(e) => handleContactFormChange('message', e.target.value)} className="w-full border-2 border-brand-dark p-2 rounded-lg bg-brand-bg focus:outline-none focus:ring-2 focus:ring-brand-orange h-32 resize-none" placeholder="Let's build something awesome..." />
                    </div>
            ...
            <div className="lg:col-span-1">
               <Card variant="white" className="h-full p-2" noShadow>
                  <EditableImage src="https://picsum.photos/seed/adam/600/800" alt="Profile Contact" className="w-full h-full object-cover rounded-lg border-2 border-brand-dark grayscale hover:grayscale-0 transition-all duration-500" storageKey="profile_contact" isEditing={isEditMode} />
               </Card>
            </div>
          </div>
        </Section>
        <footer className="bg-brand-dark text-brand-bg py-12 text-center font-bold">
